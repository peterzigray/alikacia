...
class GroupDetails extends Component {
  ...
  render () {
    const { otherProps, auth, groups } = this.props;
    if (groups) {
    var allGroups = JSON.parse(JSON.stringify(groups));
    
      var myGroup = allGroups.filter((group) => group.id === window.location.href.match(/(?!.*\/).+/)[0])[0]
          ...
          <div className="card-body">
            <table className="table table-borderless ml-3">
              <thead className="thead-inverse border-bottom">
                <th className="">
                  {myGroup ? <h3>Groups {' '} {myGroup.name}</h3> : <h3>Groups</h3>}
